  ...
  return (
    <main className="py-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div>
            ...
            <p className="mt-4 text-lg text-white/80 max-w-xl">
              Easily find verified drivers for hourly, daily, or long-term bookings. Safe, reliable, and convenient.
            </p>

            <div className="mt-8 flex flex-wrap gap-4">
              <button
                onClick={() => navigate("/sign-up")}
                className="px-7 py-3 rounded-full bg-gradient-to-r from-pink-500 via-fuchsia-600 to-indigo-500 text-white font-semibold shadow-2xl hover:brightness-105 transition"
              >
                Get started
              </button>
              <button
                onClick={() => navigate("/sign-in")}
                className="px-7 py-3 rounded-full border border-white/10 text-white/90 font-medium hover:bg-white/5 transition"
              >
                Sign in
              </button>
            </div>
            ...
            <div className="mt-8 grid grid-cols-2 gap-4 max-w-md">
              ...
              <div className="flex items-start gap-3">
                <div className="w-10 h-10 rounded-lg bg-white/5 flex items-center justify-center text-cyan-300">
                  🔒
                </div>
                <div>
              ...
              <div className="p-6">
                ...
                <div className="mt-5 rounded-lg p-4 border border-white/6 bg-white/3">
                  ...
                  <div className="mt-4 flex gap-3">
                    <button
                      onClick={() => navigate("/sign-up")}
                      className="flex-1 px-3 py-2 rounded-md bg-gradient-to-r from-pink-500 to-indigo-500 text-white font-medium hover:opacity-95 transition"
                    >
                      Request
                    </button>
                    <button
                      onClick={() => navigate("/sign-up")}
                      className="px-3 py-2 rounded-md border border-white/8 text-white/90 hover:bg-white/5 transition"
                    >
                      View
                    </button>
                  </div>
        ...
        {/* Testimonials */}
        <div className="mt-12">
          <div className="rounded-xl p-4 border border-white/6 bg-gradient-to-r from-transparent to-white/2 overflow-x-auto flex gap-4">
            {[
              { quote: "Driver arrived on time and was very professional.", name: "Asha, Bengaluru" },
              { quote: "Easy booking and transparent rates.", name: "Rahul, Delhi" },
              { quote: "Great for long-term hires.", name: "Meera, Hyderabad" },
            ].map((testimonial, index) => (
              <div
                key={index}
                className="min-w-[220px] p-3 bg-[rgba(255,255,255,0.03)] border border-white/6 rounded-lg flex-shrink-0"
              >
                <div className="text-sm text-white/80">"{testimonial.quote}"</div>
                <div className="mt-3 font-medium">— {testimonial.name}</div>
              </div>
            ))}
          </div>
        </div>
